...
void EnemyMovement::UpdateFacingVec(Enemy& enemy, EnemyStatus& status, float deltaTime)
{
    ...
    enemy.SetFacingYawForEnemy(enemy.GetGame()->GetMathUtils()->GetYawFromDirection(enemy.GetUpVec(), facingForward) + 3.14159265f);
}
...
glm::vec3 EnemyMovement::CalculateCollisionAdjustedPos(Enemy& enemy, const glm::vec3& moveDelta)
{
    ...
    if (enemy.IsAlive()) {
        desiredPos = ClampMoveToGround(enemy, desiredPos);
    }

    return desiredPos;
}

glm::vec3 EnemyMovement::ClampMoveToGround(const Enemy& enemy, const glm::vec3& desiredPos) const
{
...
bool EnemyMovement::HasGroundBelow(const Enemy& enemy, const glm::vec3& checkPos) const
{
