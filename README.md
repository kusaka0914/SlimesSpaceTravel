...
また、カメラと注視点の間に障害物がある場合は、レイキャストによってカメラ位置を補正しています。船、キー、ボス敵など、状況に応じたフォーカスカメラも実装しています。
...
弱攻撃、広範囲攻撃、強攻撃、回避、スペシャル攻撃、敵のガード破壊、打ち上げ、空中攻撃などを実装しています。
...
* `PlayerAttackHitDetector` による命中対象の検出
...
* キーボード・ゲームパッドの入力プレイヤー番号を分けて扱っている点
...
* ギズモ操作後にPhysicsを再初期化し、見た目と当たり判定のずれを防いでいる点
...
* フォーム項目を選んだときに `Game::OpenFeedbackForm` を呼ぶ点
...
* リファクタリング時は、動作を保ちながら変更理由の異なる処理を分ける
